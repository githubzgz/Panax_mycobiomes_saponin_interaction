#Figure 3 --> The fit of Sloan's Neutral Model
#The below function is adapted from Burns(2016).We lowered the initial value of m.

my_Neutral_fit <- function(comun){
  ...
  m.mle <- mle(sncm.LL, start=list(m=0.01, sigma=0.1), nobs=length(p))#We lowered the initial value of m
...
f_neutral <- my_Neutral_fit(f_norm)